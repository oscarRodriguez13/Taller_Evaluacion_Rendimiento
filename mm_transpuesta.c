...
algoritmo de multiplicación traspuesta de matrices.
*************************************/
...
int main(int argc, char *argv[]) {
  ...
  // Se le asigna memoria a las matrices, con al anterior espacio de memoria reservado
  mA = MEM_CHUNK;
  mB = mA + SZ * SZ;
  mC = mB + SZ * SZ;
  ...
  // Se crean los hilos para la multiplicación de matrices con matrices traspuestas
  for (int j = 0; j < n_threads; j++) {
    ...
    datos->nH = n_threads;
    datos->N = SZ;
    pthread_create(&p[j], &atrMM, mult_thread_mm_traspuesta, (void *)datos);
  }
